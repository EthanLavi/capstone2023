...
for img in [x for x in os.listdir("outdir") if x.__contains__(".png")]:
    # Capture frame-by-frame
    ret = True
    if restart:
        for i in range(100):
            ret, frame_crop = True, cv.imread("outdir/" + img)
        boxes = []
        h, w, c = frame_crop.shape
        empty_frame = frame_crop.copy()
    restart = True
    if ret:
        draw_frame()
        ...
        if key == ord('q'):
            ...
            os.remove("outdir/" + img)
            os.remove("outdir/" + img.replace(".png", ".txt"))
            continue
